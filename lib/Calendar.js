...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    value: function renderDays(classes) {
      ...
      var hoveredDates = _state.hoveredDates;
      ...
      return days.map(function (data, index) {
        ...
        var inHoveredRange = isInHovered(dayMoment, hoveredDates);
        ...
        return _react2['default'].createElement(_DayCellJs2['default'], _extends({
          ...
          classNames: classes,
          onItemMouseEnter: _this.dayCellHoveredHandler
        }));
      });
    }
    ...
    value: function dayCellHovered(dayMoment, startDate, endDate) {
      if (startDate.isSame(endDate, 'day')) {
        var moments = [];
        var daysDif = dayMoment.diff(startDate, 'day');
        if (daysDif === 0) return;
        if (daysDif > 0) {
          // means hovered city after selected
          for (var i = daysDif; i >= 0; i--) {

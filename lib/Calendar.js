...
function checkRange(dayMoment, range) {
  ...
  return dayMoment.isBetween(range['startDate'], range['endDate']) || dayMoment.isBetween(range['endDate'], range['startDate']) || dayMoment.isSame(range['endDate'], 'day') || dayMoment.isSame(range['startDate'], 'day');
}
...
var Calendar = (function (_Component) {
  ...
  function Calendar(props, context) {
    ...
    var isRangeError = props.isRangeError;
  ...
  _createClass(Calendar, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      var oldOpen = this.props.isOpen;
      var range = nextProps.range;
      var offset = nextProps.offset;
      var isRangeError = nextProps.isRangeError;
      var isOpen = nextProps.isOpen;

      var oldRange = this.props.oldRange;

      if (!oldOpen) {
        return this.setState({ shownDate: range['startDate'], isOpen: isOpen });
      }

      if (range && range['endDate'] && !range['endDate'].isSame(range['startDate'], "day") || oldRange && !oldRange["startDate"].isSame(range["startDate"])) {
        var _props = this.props;
        var isEndDateChanging = _props.isEndDateChanging;
        var isStartDateChanging = _props.isStartDateChanging;

        if (isEndDateChanging || !isEndDateChanging && !isStartDateChanging) {
          return this.setState({ shownDate: range['endDate'].clone().add(offset, 'months'), isRangeError: isRangeError, isOpen: isOpen });
        }
        if (isStartDateChanging) {
          return this.setState({ shownDate: range['startDate'].clone().add(offset, 'months'), isRangeError: isRangeError, isOpen: isOpen });
        }
      }
    }
  }, {
    ...
    value: function renderMonthAndYear(classes) {
      ...
      var showMonthArrow = _props5.showMonthArrow;
      var initialDate = _props5.initialDate;
      ...
      return _react2['default'].createElement(
        ...
        showMonthArrow && shownDate.isAfter(initialDate, 'month') ? _react2['default'].createElement(LeftArrow, {
        ...
        showMonthArrow && shownDate.isBefore(maxDate, 'month') ? _react2['default'].createElement(RightArrow, {
          onClick: this.changeMonth.bind(this, +1)
        }) : null
      );
    }
    ...
    value: function renderDays(classes) {
      ...
      var disableDaysBeforeToday = _props7.disableDaysBeforeToday;
      var specialDays = _props7.specialDays;
      var initialDate = _props7.initialDate;
      var onDayCellHover = _props7.onDayCellHover;
      ...
      return days.map(function (data, index) {
        ...
        var isWeekend = checkWeekend(dayMoment);
    ...
    value: function render() {
      ...
      var onlyClasses = _props8.onlyClasses;
      var classNames = _props8.classNames;
      var onCalendarOver = _props8.onCalendarOver;
      var maxRange = _props8.maxRange;
      var isRangeError = this.state.isRangeError;
      ...
      return _react2['default'].createElement(
        ...
        isRangeError && _react2['default'].createElement(
          'div',
          {
            style: styles['RangeError']
          },
          'Внимание! Максимально допустимый диапазон дат вылета — ',
          maxRange + 1,
          ' дней'
        )
      );
